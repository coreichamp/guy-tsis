...
for (let i = 1; i <= n; i++) {
  let tx_id = web3.eth.sendTransaction({
    from: account,
    data: "0x00",
  ...
  while (1) {
    let transaction = web3.eth.getTransaction(tx_id);
    if (!transaction.blockHash.includes("0x000000000000000")){
...
fs.writeFile("../n10_tx_list", json, "utf8", function(err) {
  console.log(err);
});
